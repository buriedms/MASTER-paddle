...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.data_loader):
            ...
            loss = paddle.nn.functional.cross_entropy(outputs.reshape([-1, outputs.shape[-1]]),
                                                      target[:, 1:].flatten(),  # need to remove <SOS> in target
            ...
            if self.distributed:
                ...
                reduced_metrics_tensor = paddle.to_tensor([batch_total, reduced_loss]).astype(paddle.float32)
    ...
    def _prepare_device(self, local_rank, local_world_size):
        ...
        '''
        if self.distributed:
            ngpu_per_process = len(paddle.static.cuda_places()) // local_world_size
            ...
            n_gpu = len(paddle.static.cuda_places()) if paddle.is_compiled_with_cuda() else 0

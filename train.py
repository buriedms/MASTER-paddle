...
def main(config: ConfigParser, local_master: bool, logger = None):
    ...
    val_data_loader = config.init_obj('val_loader', paddle.io,
                                      dataset = val_dataset,
                                      batch_size = 1,
    ...
    optimizer = config.init_obj('optimizer', paddle.optimizer, parameters=model.parameters())
    if config['lr_scheduler']['type'] is not None:
        lr_scheduler = config.init_obj('lr_scheduler', paddle.optimizer.lr, optimizer)
    else:
        lr_scheduler = None
...
def entry_point(config: ConfigParser):
    ...
    if config['distributed']:  # distributed gpu mode
        ...
        if paddle.device.is_compiled_with_cuda():
            if len(paddle.static.cuda_places()) < local_world_size:
                raise RuntimeError(f'the number of GPU ({len(paddle.static.cuda_places())}) is less than '
    ...
    if config['deterministic']:
        ...
        logger.warning('Actually,no setting benchmark')

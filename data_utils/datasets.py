...
def hierarchy_dataset(lmdb_dir_root, select_data = None, training = True, img_w = 256, img_h = 32, transform = None,
    ...
    if select_data is not None:
        ...
        for select_d in select_data:
            dataset = LmdbVer2Dataset(lmdb_dir_root = os.path.join(lmdb_dir_root, select_d).replace('\\','/'), training = training, img_w = img_w,
...
class LmdbVer2Dataset(Dataset):
    ...
    def __init__(self, lmdb_dir_root = None, transform = None, target_transform = None, training = True, img_w = 256,
                 img_h = 32, case_sensitive = True,
                 convert_to_gray = True):
        super(LmdbVer2Dataset, self).__init__()
        self.env = lmdb.open(lmdb_dir_root,
                             max_readers = 32,
                             readonly = True,
                             lock = False,
...
class DistCollateFn(object):
    ...
    def __call__(self, batch):
        ...
        if self.training:
            ...
            image_batch_tensor = paddle.stack(images, axis = 0).astype(paddle.float32)
...
class DistValSampler(Sampler):
    ...
    def __init__(self, indices, batch_size, distributed = True):
        super(DistValSampler, self).__init__()
